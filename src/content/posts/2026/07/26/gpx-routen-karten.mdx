...
Für Fahrrad-Touren (Tag `fahrrad`) wollte ich nicht nur einen Text schreiben, sondern die aufgezeichnete Strecke direkt als Karte zeigen — inklusive Höhenprofil. Ein Beispiel dafür ist die [Dresden-Moritzburg Runde](/dresden-moritzburg-runde). Hier kurz, wie das technisch funktioniert.
...
Welche GPX-Datei zu welchem Post gehört, steht dagegen nicht im Frontmatter, sondern wird im Post selbst über die `RouteMap`-Komponente angegeben — einfach der Dateiname:
...
Da die GPX-Datei immer im selben Ordner wie der Post liegt, reicht der Dateiname. Beim Rendern wird er mit dem Ordner des Posts kombiniert, um den passenden Eintrag aus der `activities`-Collection zu finden.
